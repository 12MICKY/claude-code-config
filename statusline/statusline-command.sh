...
if [ -n "$tok_total" ]; then
  if [ -n "$week_pct" ] && [ "${tok_7d_raw:-0}" -gt 0 ] 2>/dev/null; then
    TOK_MAX=$(awk -v raw="$tok_7d_raw" -v pct="$week_pct" \
      'BEGIN{printf "%.0f", raw / (pct/100)}')
  fi
  tok_pct=$(awk -v raw="${tok_raw:-0}" -v max="$TOK_MAX" \
    'BEGIN{p=raw/max*100; if(p>100)p=100; printf "%.1f", p}')
  parts+=("$(progress_bar "$tok_pct" "🅰:" "")")
fi
